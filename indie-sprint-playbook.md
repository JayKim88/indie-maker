...
```
[Phase 0+1]  [Phase 1.5]  [Phase 2]   [Phase 3+4]       [Phase 5]       [Phase 6]  [Phase 7]    [Gate]     [Phase 8+]  [Phase 9]
  Idea +    → UX Sprint → Design   → Build +         → Launch        → Launch  → Post      → Kill/Go → Growth  → Retro
  Planning               Sprint     Deploy Sprint     Prep Sprint      Day       Launch                  Sprint     Sprint
  D1 (1일)   D1 오후     D2 (1일)   D3-D6 (4일)       D7-D13 (7일)     D14       D15-D28      D29        D30+        D29(Kill)
...
|------|-------------|----------------|------|
| PRD + 시장분석 | 1-2일 | 2-4시간 | `/business-avengers` 자동 생성 |
| 랜딩 카피 + 디자인 방향 | 2-3일 | 반나절 | `/launch-kit` + v0.dev |
...
### D1 오전 (2-3시간): Idea Canvas

- [ ] **문제 명확화**: 누구의 어떤 문제인가? (나 자신 포함)
- [ ] **기존 해결책**: 지금 사람들은 어떻게 해결하나? 왜 부족한가?
- [ ] **독특한 각도**: 내가 유리한 이유 1가지
- [ ] **Kill 기준 설정**: D29 기준, 어떤 숫자면 계속할 것인가?

```bash
# 아이디어 즉시 검증 (10분)
/business-avengers ask cpo "Validate this idea: [your idea in 1 sentence]"
```

### D1 오후 (4-5시간): Planning Sprint

```bash
# PRD + 시장분석 + 경쟁사 분석 + 기술 아키텍처 + 비즈니스 모델 자동 생성
/business-avengers new --mode mvp-build "[idea]"
# → prd-lean.md, competitive-brief.md, tech-stack.md, business-model.md 자동 생성

# 비즈니스 모델 추가 검토 (필요시)
/business-avengers ask cfo "Best pricing model for [product type], targeting [user type]"
```
...
| 파일 | 생성 방식 |
|------|---------|
| `idea-canvas.md` | 직접 작성 (30분) |
| `prd-lean.md` | `/business-avengers` 자동 생성 → 검토/수정 |
| `competitive-brief.md` | `/business-avengers` 자동 생성 |
| `tech-stack.md` | 권장 스택 사용 (변경 이유 없으면 그대로) |
| `business-model.md` | `/business-avengers` 초안 → 직접 확정 |
...
```bash
# 랜딩 카피 + PH 콘텐츠 + 이메일 시퀀스 일괄 생성 (30분)
/launch-kit
# → landing-copy.md, PH 첫 코멘트, 태그라인 자동 생성
```
...
| 파일 | 생성 방식 |
|------|---------|
| `design-brief.md` | 컬러 hex + 폰트명 + 로고 방식 (직접, 15분) |
| `landing-copy.md` | `/launch-kit` 자동 생성 → 검토 |
| `wireframes/` | v0.dev 스크린샷 or 손그림 (빠르게) |
| `components.md` | 필요 컴포넌트 목록 (직접, 10분) |
...
| Phase 5 런치 준비 | `/indie-launcher` | PH 전략 + BIP 캘린더 + 커뮤니티 공략 | 4-6시간 |
...
| 단계 | 현재 | 목표 자동화 |
|------|------|-----------|
| D1 오전: 아이디어 검증 | 직관 (1-2시간) | `/business-avengers ask cpo` (10분) |
| D1 오후: 시장 분석 + PRD | 수동 4-8시간 | `/business-avengers new --mode mvp-build` (30분) |
| D2 오전: 랜딩 카피 | 수동 3-5시간 | `/launch-kit` (30분) |
